...
if (!LAST_BASE_RULE) {
    ...
    OUTPUT_PATH = file.path(RECESSIVE_DIR,"results/recessive.allele_frequency_tests.autozygosity.txt")
} else {
    ...
    OUTPUT_PATH = file.path(RECESSIVE_DIR,"results/recessive.allele_frequency_tests.last_base_rule.autozygosity.txt")
}
