...
analyse_inherited_enrichment <- function(hgnc, chrom, biallelic_lof, biallelic_func, biallelic_silent, lof_func, probands=NULL, cohort_n=3072, check_last_base=FALSE, autozygous_rate=0) {
    ...
    if (class(ddd) != "try-error") {
        ...
        ddd = test_enrichment(ddd, biallelic_lof, biallelic_func, biallelic_silent, lof_func, sum(unlist(cohort_n)), autozygous_rate)
    } else {
    ...
    if (!is.list(cohort_n)) {
        exac = test_enrichment(exac[["NFE"]], biallelic_lof, biallelic_func, biallelic_silent, lof_func, cohort_n, autozygous_rate)
    } else {
        exac = test_enrichment_across_multiple_populations(exac, biallelic_lof, biallelic_func, biallelic_silent, lof_func, cohort_n, autozygous_rate)
    }
...
test_enrichment <- function(freq, biallelic_lof, biallelic_func, biallelic_silent, lof_func, cohort_n, autozygous_rate=0) {
    ...
    freq$biallelic_silent_p = biallelic_silent_enrichment(freq, biallelic_silent, cohort_n, autozygous_rate)
...
#'
#' @return P-value from testing for biallelic functional variants.
biallelic_silent_enrichment <- function(freq, count, cohort_n, autozygous_rate=0) {
    rate = (freq$synonymous ** 2) * (1 - autozygous_rate) + freq$functional * autozygous_rate
    return(pbinom(count - 1, cohort_n, prob=rate, lower.tail=FALSE))
}
...
#'         functional tests.
test_enrichment_across_multiple_populations <- function(exac, biallelic_lof, biallelic_func, biallelic_silent, lof_func, cohort_n, autozygous_rate) {
    ...
    biallelic_silent_combos = get_count_combinations(populations, biallelic_silent)
    ...
    p_values$biallelic_silent_p = sum_combo_tests(exac, cohort_n,
        biallelic_silent_combos, biallelic_silent_enrichment, autozygous_rate)
